...
package forester.paging.terraingrid;
...
/**
 * Terraingrid-based paging manager. Not finished yet (version 1.0).
 * @author Andreas
 */
public class TerrainGridPagingManager implements TerrainGridListener, PagingManager{
    
    protected static final Logger log = Logger.getLogger(TerrainGridPagingManager.class.getCanonicalName());
     ...
     * Constructor.
     * 
     * @param terrainGrid the terraingrid.
     ...
     * @param camera A jME <code>Camera</code> object.
     */
    public TerrainGridPagingManager(TerrainGrid terrainGrid, int pageSize, int resolution, Node rootNode, Camera camera){
        ...
        pagingNode.setLocalTranslation(terrainGrid.getWorldTranslation().clone());
    ...
    public void tileDetached(Vector3f cell, TerrainQuad quad) {
        int x = (int) cell.x;
        int z = (int) cell.z;
        Page page = grid.removeCell(x,z);
        if(useCache){
            cache.add(page);
        } else {
            page = grid.removeCell(x,z);
            page.unload();
        }
    }
